...
- Build/deploy a root `Containerfile` derived from an exact semver, e.g. `FROM ghcr.io/aakarim/openlore:1.2.3`; no floating tags. Keep the derivative for future package customization.
- Root `openlore.yml` is authoritative and must set `auth_file: /var/lib/openlore/config/lore.json`, `writable_dir: /var/lib/openlore/published`, `data_dir: /var/lib/openlore/data`, and `host_key_path: /var/lib/openlore/ssh/openlore_ed25519`, with OpenLore SSH internal port `2222` and the configured HTTP port.
...
- Create one Compute Engine VM, attach one persistent disk, mount it at `/var/lib/openlore` via stable disk identity, and ensure the derivative container restarts after container/VM reboot.
...
Inspect the mounted disk over `gcloud compute ssh`. Transfer `.local/lore.json` and `.local/filesystem` only if remote `/var/lib/openlore/config/lore.json` is absent and published/data state is empty. Create `config`, `published`, `data`, and `ssh` securely. Stop when any remote state exists; never overwrite or merge it.
